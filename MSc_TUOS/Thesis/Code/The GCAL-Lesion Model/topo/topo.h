...
public:
    ...
    Projection(std::vector<Flt *> fSrc, std::vector<Flt *> fDst, morph::HexGrid *hgSrc, morph::HexGrid *hgDst,
               Flt radius, Flt strength, Flt alpha, Flt sigma, bool normalizeAlphas, Flt jitter) {
...
#pragma omp parallel for
    ...
    // added for Zheng Yuan
    Projection(std::vector<Flt *> fSrc, std::vector<Flt *> fDst, morph::HexGrid *hgSrc, morph::HexGrid *hgDst,
               Flt radius, Flt strength, Flt alpha, Flt sigma, bool normalizeAlphas, Flt jitter, Flt lesionX,
               Flt lesionY, Flt lesionRadius) {
        ...
        Flt radiusSquared = radius * radius;    // precompute for speed
        Flt radiusLesionSquared = lesionRadius * lesionRadius;
...
#pragma omp parallel for
        for (unsigned int i = 0; i < nDst; i++) {
            ...
            // next 3 lines implement a circular lesion on the src sheet
            Flt dxLesion = hgSrc->vhexen[i]->x + jitterx - lesionX;
            Flt dyLesion = hgSrc->vhexen[i]->y + jittery - lesionY;
            if (dxLesion * dxLesion + dyLesion * dyLesion >= radiusLesionSquared) {
                for (unsigned int j = 0; j < nSrc; j++) {
                    Flt dx = (hgSrc->vhexen[j]->x + jitterx - hgDst->vhexen[i]->x);
                    Flt dy = (hgSrc->vhexen[j]->y + jittery - hgDst->vhexen[i]->y);
                    Flt distSquared = dx * dx + dy * dy;
...
public:
    ...
    void addProjection(std::vector<Flt *> inXptr, morph::HexGrid *hgSrc, float radius, float strength, float alpha,
                       float sigma, bool normalizeAlphas) {
    ...
    void addProjection(std::vector<Flt *> inXptr, morph::HexGrid *hgSrc, float radius, float strength, float alpha,
                       float sigma, bool normalizeAlphas, float jitter) {
        Projections.push_back(
                Projection<Flt>(inXptr, this->Xptr, hgSrc, this->hg, radius, strength, alpha, sigma, normalizeAlphas,
                                jitter));
    }
    ...
    // Added for Zheng Yuan
    void addProjection(std::vector<Flt *> inXptr, morph::HexGrid *hgSrc, float radius, float strength, float alpha,
                       float sigma, bool normalizeAlphas, float jitter, float lesionX, float lesionY,
                       float lesionRadius) {
        Projections.push_back(
                Projection<Flt>(inXptr, this->Xptr, hgSrc, this->hg, radius, strength, alpha, sigma, normalizeAlphas,
                                jitter, lesionX, lesionY, lesionRadius));
    }
...
#pragma omp parallel for
        for (unsigned int hi = 0; hi < this->nhex; ++hi) {
            ...
            this->X[hi] =
                    amplitude * exp(-((dx * cosTheta - dy * sinTheta) * (dx * cosTheta - dy * sinTheta)) * overSigmaA
                                    - ((dx * sinTheta + dy * cosTheta) * (dx * sinTheta + dy * cosTheta)) * overSigmaB);
        }
    }


    void Gaussian(std::vector<double> x_center, std::vector<double> y_center, std::vector<double> theta,
                  std::vector<double> sigmaA, std::vector<double> sigmaB, std::vector<double> amplitude) {
...
#pragma omp parallel for
            for (unsigned int hi = 0; hi < this->nhex; ++hi) {
                ...
                double val = amplitude[i] *
                             exp(-((dx * cosTheta - dy * sinTheta) * (dx * cosTheta - dy * sinTheta)) * overSigmaA
                                 - ((dx * sinTheta + dy * cosTheta) * (dx * sinTheta + dy * cosTheta)) * overSigmaB);
                if (val > this->X[hi]) {
                    this->X[hi] = val;
                }
...
#pragma omp parallel for
        for (unsigned int hi = 0; hi < this->nhex; ++hi) {
            ...
            this->X[hi] = amplitude * sin(width *
                                          (this->hg->vhexen[hi]->x * sinTheta + this->hg->vhexen[hi]->y * cosTheta +
                                           phase));
    ...
    void CircleStimulus(double x, double y, double radius) {

#pragma omp parallel for
        for (unsigned int hi = 0; hi < this->nhex; ++hi) {
            double dx = this->hg->vhexen[hi]->x - x;
            double dy = this->hg->vhexen[hi]->y - y;
            this->X[hi] = ((dx * dx + dy * dy) < radius * radius) * 1.0;
        }
    }

    void GaussianStimulus(double x, double y, double sigma) {

#pragma omp parallel for
        for (unsigned int hi = 0; hi < this->nhex; ++hi) {
            double dx = this->hg->vhexen[hi]->x - x;
            double dy = this->hg->vhexen[hi]->y - y;
            this->X[hi] = exp(-(dx * dx + dy * dy) / (sigma * sigma)) * 1.0;
        }
    }

    void SquareStimulus(double x, double y, double width) {

#pragma omp parallel for
        for (unsigned int hi = 0; hi < this->nhex; ++hi) {
            double dx = this->hg->vhexen[hi]->x - x;
            double dy = this->hg->vhexen[hi]->y - y;
            this->X[hi] = (fabs(dx) < width) * (fabs(dy) < width) * 1.0;
        }
    }
};
...
#pragma omp parallel for
    ...
    */
    ...
    void stepPreloaded(int p) {
        ...
        int k = 0;
        for (int i = 0; i < C.nx; i++) { // 90
            for (int j = 0; j < C.ny; j++) { //90
                int ind = (offy + i) * patternsWid + offx + j; // [num + i]*256 > (256*256)
                C.vsquare[k].X = PreLoadedPatterns[p][ind]; //PreLoadedPatterns [25] * [256*256]
                k++;
            }
        }
        step();
    ...
    void stepPreloaded(int p, float offx,
                       float offy) { // should supply xoff,yoff in sheet coords and convert inside this function really.
        int k = 0;
        for (int i = 0; i < C.nx; i++) {
            for (int j = 0; j < C.ny; j++) {
...
public:
    ...
    void initProjection(int nx, int ny, float rangex, float rangey, std::vector<Flt *> fSrc, morph::HexGrid *hgSrc,
                        Flt radius, Flt sigma) {
...
#pragma omp parallel for
    ...
    std::vector<std::vector<float> > fft(cv::Mat I, int nbins, int gaussBlur, bool view) {
        ...
        cv::Mat splitArray[2] = {cv::Mat::zeros(dftOfOriginal.size(), CV_32F),
                                 cv::Mat::zeros(dftOfOriginal.size(), CV_32F)};
