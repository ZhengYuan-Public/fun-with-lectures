...
def load_data_matrix(file_path):
    with open(file_path) as file_object:
        contents = pd.read_csv(file_path, sep='\t')
    return contents
...
df = load_data_matrix(data_set_path)
# print('\nVisualize Original DataSet:\n',df)
# print('-'*100)

# Step 0: Calculate MIC for each gene
f_array = df.iloc[:, 1:].to_numpy()
# Feature array, converted to numpy array
c_array = df.columns.str.contains('POS*') == True
c_mic = c_array[1:]
# Class array, boole

MIC = []
# MIC array stores the MIC value for each row(gene)
f_sel_index = []
# The index for selected feature
...
df_new = df[['mdr']].copy()
df_new['MIC_VALUE'] = MIC
# Create a new DataFrame with only Gene names and its MIC values respectively.
df_new_selected = df_new.iloc[f_sel_index, :]
# Use the f_sel_index to choose the features from Step 1
df_new_selected_sorted = df_new_selected.sort_values(by='MIC_VALUE',ascending = False)
# Sort it into Descending Order
f_sel_index_sorted = df_new_selected_sorted.index
# New index for selected&sorted features
